...
date <- readline(prompt="Enter date (dd-mm-yyyy): ")
days_diff<- function(sales_df,date){
  #3. Create new column 'days_diff'
  sales_df$days_diff<- as.Date(date,"%d-%m-%Y")-as.Date(sales_df$OrderDate)
  print(head(df$days_diff))
}
...
slsmn_units<- function(df){
  ...
  df3<- df %>% group_by(Region) %>% summarize(Total.sales=sum(Sale_amt,na.rm=T),SalesMan.count=length(unique(SalesMan)))
  print(df3)
}
...
sales_pct<-function(df){
  ...
  l2<- sum(df$Sale_amt)
  print(df %>% group_by(Manager) %>% summarize(percent.sales=(sum(Sale_amt,na.rm=T)/l2)*100))
}
...
drop_row<- function(diamonds_df){
  ...
  diamonds_df1<- diamonds_df[complete.cases(diamonds_df$carat),]
  diamonds_df1<- diamonds_df1[complete.cases(diamonds_df$cut),]
  print(diamondsdf1)
}
